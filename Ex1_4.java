...
class Dist{
    double n = 0;
    int[] vertex = new int[1500];
    int count_vertex_d = 0;
    int p = -1; //parent
}
    ...
    public static void main(String args[]){
        ...
        for(i=0;i<M*2;i=i+2){
            for(j=0;j<M*2;j=j+2){
                if(i == j) continue;
                    ...
                    if((0<s && s<1) && (0<t && t<1)){
                        // 交差あり step 4
                        int frag = 0; //すでに座標があるかどうか判定
                        double d_x = p1_x + (q1_x - p1_x) * s;
                        String tmp_x = String.valueOf(d_x);
                        double d_y = p2_y + (q2_y - p2_y) * t;
                        String tmp_y = String.valueOf(d_y);
                        BigDecimal bd_x = new BigDecimal(tmp_x); //座標で交差座標の存在を確認するためBigDecimal型のsetScaleを用いて小数点第11位を四捨五入する
        ...
        for(i=0;i<Q;i++){
            ...
            // ダイクストラ
                ...
                double min = 0;
                Dist[] dist = new Dist[N + count_intersection];
                for(int k=0;k<N+count_intersection;k++){
                    dist[k] = new Dist();
                }
                ...
                while(true){
                    ...
                    for(int v=0;v<num_vertex;v++){
                        if(color[v] != BLACK && array_dijkstra[u][v] != INFTY){
                            if(dist[v].n > dist[u].n+ array_dijkstra[u][v]){
                                dist[v].n = dist[u].n + array_dijkstra[u][v];
                                dist[v].p = u;
                                color[v] = GRAY;
                            }
                        }
                ...
                int[] arr_p = new int[N+count_intersection];
                for(j=0;j<N+count_intersection;j++){
                    arr_p[j] = dist[j].p;
                }
                
                
                int[] way_s_e = new int[1500]; //経路の番号を保存するための配列
                int count_s_e = 0; //way_s_eのインデックス操作のための変数
                while(true){
                    if(count_s_e==0){
                        way_s_e[count_s_e++] = end;
                        
                        way_s_e[count_s_e++] = arr_p[end];
                        
                    }
                    
                    way_s_e[count_s_e] = arr_p[way_s_e[count_s_e-1]];
                    count_s_e++;
                    if(way_s_e[count_s_e-1] == start) break;
                    

                }
                
                String[] str_way = new String[count_s_e]; //経路の番号を保存するための配列

                //それぞれに+1
                //元の座標と交差座標の判別
                for(j=0;j<count_s_e;j++){
                    way_s_e[j]++;
                    if(way_s_e[j] > N){
                        int tmp_v = way_s_e[j]-(N);
                        String tmp_w = "C"+String.valueOf(tmp_v);
                        str_way[j] = tmp_w;
                    }
                    else{
                        String tmp_w = String.valueOf(way_s_e[j]);
                        str_way[j] = tmp_w;
                    }
                }
                //経路を配列の逆順から出力(配列の中身が経路の出力とは逆に入れてしまったため)
                for(j=count_s_e-1;j>=0;j--){
                    System.out.print(str_way[j]+" ");
                }
                System.out.println("");
